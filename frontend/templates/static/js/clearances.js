document.addEventListener("DOMContentLoaded", () => {
    ...
    const filterGender = document.getElementById("filterGender"); // Filter dropdown
    let allClearances = []; // Store all clearances for filtering
    ...
    searchInput.addEventListener("input", () => {
      ...
      const filteredClearances = allClearances.filter(clearance =>
        clearance.name.toLowerCase().includes(query) || clearance.dob.toLowerCase().includes(query)
      );
      displayClearances(filteredClearances);
    });
  
    // Event listener for filter
    filterGender.addEventListener("change", () => {
      const gender = filterGender.value;
      const filteredClearances = gender
        ? allClearances.filter(clearance => clearance.gender === gender)
        : allClearances;
      displayClearances(filteredClearances);
    });
    ...
    async function fetchClearances() {
      ...
      try {
        ...
        if (response.ok) {
          allClearances = await response.json(); // Store for filtering
          displayClearances(allClearances);
